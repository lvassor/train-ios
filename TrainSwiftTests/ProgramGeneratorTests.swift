...
final class ProgramGeneratorTests: XCTestCase {
    ...
    func testGetStrongerOnlyHighRating() throws {
        // High rating (>75) with "get_stronger" only → should be "5-8" or "6-10"
        let range = try generator.getRepRangeForGoalAndRating("get_stronger", canonicalRating: 80)
        XCTAssertTrue(["5-8", "6-10"].contains(range),
                       "Expected '5-8' or '6-10' but got '\(range)'")
    }

    func testGetStrongerOnlyLowRating() throws {
        // Low rating (≤75) with "get_stronger" only → should be "6-10" or "8-12"
        let range = try generator.getRepRangeForGoalAndRating("get_stronger", canonicalRating: 50)
        XCTAssertTrue(["6-10", "8-12"].contains(range),
                       "Expected '6-10' or '8-12' but got '\(range)'")
    }

    func testIncreaseMuscleAndFatLoss() throws {
        // "increase_muscle" + "fat_loss" → should be "8-12" or "10-14"
        let range = try generator.getRepRangeForGoalAndRating("increase_muscle,fat_loss", canonicalRating: 60)
        XCTAssertTrue(["8-12", "10-14"].contains(range),
                       "Expected '8-12' or '10-14' but got '\(range)'")
    }

    func testFatLossOnly() throws {
        // "fat_loss" only → should be "8-12" or "10-14"
        let range = try generator.getRepRangeForGoalAndRating("fat_loss", canonicalRating: 50)
        XCTAssertTrue(["8-12", "10-14"].contains(range),
                       "Expected '8-12' or '10-14' but got '\(range)'")
    }

    func testIncreaseMuscleOnly() throws {
        // "increase_muscle" only → should be "6-10" or "8-12"
        let range = try generator.getRepRangeForGoalAndRating("increase_muscle", canonicalRating: 50)
        XCTAssertTrue(["6-10", "8-12"].contains(range),
                       "Expected '6-10' or '8-12' but got '\(range)'")
    }

    func testDefaultGoalRepRange() throws {
        // Unknown goal → should default to "8-12"
        let range = try generator.getRepRangeForGoalAndRating("unknown_goal", canonicalRating: 50)
        XCTAssertEqual(range, "8-12")
    }

    func testGetStrongerWithFatLoss() throws {
        // "get_stronger" + "fat_loss" → treated as strength-focused
        let range = try generator.getRepRangeForGoalAndRating("get_stronger,fat_loss", canonicalRating: 80)
        XCTAssertTrue(["5-8", "6-10"].contains(range),
                       "Expected '5-8' or '6-10' but got '\(range)'")
    }
    ...
    func testRepRangesAreAlwaysValid() throws {
        ...
        for goal in goals {
            for rating in ratings {
                for _ in 0..<5 {
                    let range = try generator.getRepRangeForGoalAndRating(goal, canonicalRating: rating)
